...
test('updates the published payload when the selected stage changes', async ({ page }) => {
  ...
  await expect(page.getByText('Payload allowance: 785 kg')).toBeVisible();
  await page.getByLabel(/Stage 1: The Weekender/i).check();
  await expect(page.getByText('Payload allowance: 1740 kg')).toBeVisible();
  await page.getByLabel(/Stage 2: Apex Series/i).check();
  await expect(page.getByText('Payload allowance: 2120 kg')).toBeVisible();
...
test('checks an entered payload against the selected stage without changing GVM', async ({
  page,
}) => {
  await page.goto(FIXTURE_URL);
  await selectVehicle(page, 'chevrolet_silverado_1500');
  ...
  const payloadInput = page.getByRole('spinbutton', {
    name: 'Payload to check',
    exact: true,
  });
